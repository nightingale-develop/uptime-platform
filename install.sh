...
if ((CLEAN == 0)); then
  ...
  if [[ -f "$INSTALL_DIR/.env" ]]; then
    ...
    curl --fail --silent --show-error --location --proto '=https' --tlsv1.2 \
      --connect-timeout 15 --max-time 120 "$RELEASE_BASE/main/update.sh" -o "$stage/update.sh"
    bash -n "$stage/update.sh"
    flock -u 9
    bash "$stage/update.sh" --version "$RELEASE_VERSION"
    exit 0
  fi
...
for file in "${release_files[@]}"; do
  ...
  curl --fail --silent --show-error --location --proto '=https' --tlsv1.2 \
    --connect-timeout 15 --max-time 120 "$RELEASE_BASE/v$RELEASE_VERSION/$file" -o "$stage/$file"
done
curl --fail --silent --show-error --location --proto '=https' --tlsv1.2 \
  --connect-timeout 15 --max-time 120 "$RELEASE_BASE/main/update.sh" -o "$stage/update.sh"
bash -n "$stage/update.sh"
